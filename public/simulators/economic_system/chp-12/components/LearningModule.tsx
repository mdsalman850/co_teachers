...
import { Home, Sun, Factory, Truck, Coins, Globe, Wheat, Shirt, ShoppingBag, ArrowRight, ArrowLeft, Scale, Heart, Lightbulb, TrendingUp, Users, BookOpen, MapPin, Building, Shield, Target, BarChart3, Zap } from 'lucide-react';
...
export const LearningModule: React.FC = () => {
      ...
      content: (
        <div className="space-y-4 md:space-y-6 animate-in fade-in duration-500">
          ...
          <div className="relative pl-4 md:pl-6">
            {/* Connecting Line for this mini-diagram */}
  ...
  return (
    <div className="h-full w-full flex flex-col items-center pt-4 pb-4 px-4 md:px-6">
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-xl border border-slate-200 flex flex-col flex-1 min-h-0 overflow-hidden relative transition-all duration-300">
        ...
        {/* Card Header (Fixed) */}
        <div className={`bg-${currentStepData.color}-50 px-5 py-3 md:px-6 md:py-4 border-b border-${currentStepData.color}-100 flex items-center gap-3 md:gap-4 shrink-0`}>
          <div className={`p-2 md:p-3 rounded-xl bg-${currentStepData.color}-500 text-white shadow-md transform -rotate-3 transition-transform hover:rotate-0`}>
        ...
        <div className="bg-slate-50 px-5 py-3 border-t border-slate-100 flex justify-between items-center shrink-0 z-10">
          <button 
            ...
            className={`flex items-center gap-2 font-bold px-3 md:px-5 py-2 rounded-xl transition-all text-xs md:text-sm ${currentSlide === 0 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-600 hover:bg-slate-200 hover:text-slate-900'}`}
          >
